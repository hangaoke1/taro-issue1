...
export default function Index(props) {
  ...
  function handleClick(item) {
    ...
    if (item.type === 'url') {
      ...
      if (disableList.includes(valid_operation)) {
        return;
      }
      // 判断是否是机器人
      if (!get('isRobot')) {
        setDisableList([...disableList, valid_operation]);
  ...
  return tpl ? (
    <View className="m-order-status">
      ...
      <View className="u-list">
        {list.map(item => (
          <View
            className={`u-list-item ${disableList.includes(item.valid_operation) ? 'z-disabled' : ''}`}
            onClick={handleClick.bind(null, item)}
            key={item.valid_operation}
          >
            {item.valid_operation}
          </View>
        ))}
      </View>
    </View>
