...
export default function MessageView(props) {
  ...
  return it ? (
    <View>
      ...
      {['text', 'rich'].includes(it.type) ? (
        <TextView item={it}></TextView>
      ) : null}

      {['action', 'entries'].includes(it.type) ? (
        <ActionView item={it}></ActionView>
      ) : null}
      {it.type === 'image' ? (
        <ImgView onClick={handleImgClick} item={it}></ImgView>
      ) : null}
      {it.type === 'audio' ? <AudioView item={it}></AudioView> : null}
      {it.type === 'video' ? <VideoView item={it}></VideoView> : null}
      {['qa-list', 'qa'].includes(it.type) ? (
