...
class Chat extends Component {
  ...
  scrollToBottom = (scrollWithAnimation = true, delay = 300, force = true) => {
    if (this.state.scrollIntoView !== '' && this.state.scrollIntoView !== 'm-bottom' && !force) {
      return
    }
  ...
  handleFuncClick = (item) => {
    const { sendImage: _sendImage, sendVideo: _sendVideo } = this.props
    switch (item.type) {
      case 'album':
        // 照片
        Taro.chooseImage({
  ...
  handleQuickEntryClick = (entry) => {
    ...
    setTimeout(() => {
      this.lockEntrance = this.lockEntrance.filter((item) => item !== entry.label)
    }, 1000)
    ...
    switch (entry.action) {
      ...
      case 'evaluate':
        ...
        let evaluation_timeout =
          (Session.shop.setting && Session.shop.setting.evaluation_timeout * 60 * 1000) || 10 * 60 * 1000
  ...
  // 点击联想文本
  handleAssociateClick = _debounce(
    (text) => {
      if (!text.trim()) {
        return
      }
  ...
  render () {
    const { Message, Options, CorpStatus, Bot, Associate, Session, Setting } = this.props
    const {
      height,
      videoUrl,
      showAssociate,
    ...
    return (
      <Index className='m-page-wrapper'>
        <Back />
        <View class='u-loading' style={`visibility: ${showLoading ? 'visible' : 'hidden'}`}>
          消息加载中...
        </View>
        {/* 视频全局对象 */}
        <View
          style={`display: ${videoUrl
            ? 'block'
            : 'none'};position:fixed;top:0;bottom:0;right:0;left:0;z-index:999;background-color:#000;`}>
          <Video
            id='j-video'
        ...
        <View className='m-chat'>
          <ScrollView
            ...
            <View style={`height: ${showTopPlaceHolder ? constOffset : 0}px`} />
            {isFullScreen && <View style={`height: ${statusBarHeight}px`} />}
            {Message.map((it) => (
              <View key={it.uuid} id={it.uuid} style={{ position: 'relative' }}>
                <MessageView it={it} onImgClick={this.handleImgClick} />
                {/* TODO: 增加配置开关判断 */}
                {it.fromUser && it.withKefu ? (
                  <View class='u-read'>
                    {it.time < Session.readTime ? <Text>已读</Text> : <Text style={{ color: '#337eff' }}>未读</Text>}
                  </View>
                ) : null}
              </View>
            ))}
            <View style={`height: ${hasBot || hasQuickEntry ? Taro.pxTransform(76) : 0}`} />
            <View id='m-bottom' />
          </ScrollView>
        </View>
        ...
        <View
          className={`u-chatbox`}
          style={`bottom: ${height ? height + 'px' : isOpen ? Taro.pxTransform(544) : '0'};`}>
          {showAssociate ? (
                ...
                {Associate.questionContents.map((item) => {
                  return (
                    <View className='m-associate-item' onClick={this.handleAssociateClick.bind(this, item.value)}>
                      <RichText nodes={text2em(item.value, Associate.content)} />
                    </View>
                  )
                })}
              </View>
            ...
            <ScrollView scrollX className='m-bot'>
              {Bot.botList.map((bot, index) => (
                <View
                  ...
                  style={`border-color: ${Setting.themeColor || '#e1e3e6'};animation-delay: ${index * 200}ms;`}
                  key={bot.id}
        ...
        <View className='u-mask' style={`height: ${height}px`} />
        <View className={`u-funcbox ${Options.showFunc ? 'open' : ''}`}>
          <FuncBox list={functionList} onFuncClick={this.handleFuncClick} />
        </View>
        <View className={`u-funcbox ${Options.showPortrait ? 'open' : ''}`}>
          <Portrait onEmojiClick={this.handleEmojiClick} />
        </View>
        <FloatLayout
        ...
        {CorpStatus.entryConfig.length ? (
          <FloatButton entryConfig={CorpStatus.entryConfig} onSelect={this.handleSelectEntry} />
        ) : null}
      </Index>
    )
  }
